...
int16_t Acc_rawX, Acc_rawY, Acc_rawZ,Gyr_rawX, Gyr_rawY, Gyr_rawZ;
 float Acceleration_angleY, Acceleration_angleX;
float Gyro_angleY,Gyro_angleX;
float Total_angleY,Total_angleX;
float elapsedTime, time, timePrev;
float desired_angle = 0;
double throttle=1000;//initial value of throttle to the motors
double PITCHcom=1500; //initial value of angle to the servos
double ROLLcom=1500;
double ROLLvalright=1500;
double ROLLvalleft=1500;
double YAWcom=1500;
int i;
float rad_to_deg = 180/3.141592654;

float PID, pwmLeft, pwmRight, error, previous_error;
float PIDX, pwmPITCHright, pwmPITCHleft, errorX, previous_errorX;
float pid_pX=0;
float pid_iX=0;
float pid_dX=0;
float pid_p=0;
...
void setup()
{
    Wire.begin(); //begin the wire comunication pour l'accéléromètre
    ...
    right_prop.attach(5); //attach the right motor to pin 5
    left_prop.attach(6);  //attach the left motor to pin 6
    right_servo.attach(3); //attach the right servo to pin 3
    left_servo.attach(2);  //attach the left servo to pin 2
   ...
   * begin functions we have to put this value.*/
    ...
    */
     ...
     *  will calculate the rooth square.*/
     /*---X---*/
     Acceleration_angleY = atan((Acc_rawY/16384.0)/sqrt(pow((Acc_rawX/16384.0),2) + pow((Acc_rawZ/16384.0),2)))*rad_to_deg;
     /*---Y---*/
     Acceleration_angleX = atan(-1*(Acc_rawX/16384.0)/sqrt(pow((Acc_rawY/16384.0),2) + pow((Acc_rawZ/16384.0),2)))*rad_to_deg;
   ...
   Gyro_angleY = Gyr_rawY/131.0;
    /*---X axis angle---*/
   Total_angleX = 0.98 *(Total_angleX + Gyro_angleY*elapsedTime) + 0.02*Acceleration_angleX;
   /*---Y axis angle---*/
   Total_angleY = 0.98 *(Total_angleY + Gyro_angleX*elapsedTime) + 0.02*Acceleration_angleY;
...
error = Total_angleY - desired_angle;
...
throttle=pulse_duration[CHANNEL3];
PITCHcom = pulse_duration[CHANNEL2]; 

ROLLcom =pulse_duration[CHANNEL1];
if(ROLLcom>1700){
  ROLLcom=1700;
}
if(ROLLcom<1300){
...
YAWcom = pulse_duration[CHANNEL4];
if(YAWcom>1700){
  YAWcom=1700;
}
if(YAWcom<1300){
...
pwmPITCHright = PITCHcom - PIDX - 195;
pwmPITCHleft =(3000- PITCHcom) + PIDX + 30;
...
//PITCHright
if(pwmPITCHright < 1015)
{
  pwmPITCHright= 1015;
}
if(pwmPITCHright > 1770)
{
  pwmPITCHright=1770;
}
//PITCHleft
if(pwmPITCHleft < 1027)
{
  pwmPITCHleft= 1027;
}
if(pwmPITCHleft > 1910)
{
  pwmPITCHleft=1910;
}
...
right_servo.writeMicroseconds(pwmPITCHright);
left_servo.writeMicroseconds(pwmPITCHleft);

if((YAWcom>1480)&&(YAWcom<1520)){
  right_servo.writeMicroseconds(pwmPITCHright);
  left_servo.writeMicroseconds(pwmPITCHleft);
  }
  else{
    right_servo.writeMicroseconds(YAWcom -195);
    left_servo.writeMicroseconds(YAWcom + 30);
  }

  
  ROLLvalleft=throttle+(ROLLcom-1500);
  ROLLvalright=throttle-(ROLLcom-1500);
  ...
  if(ROLLvalright<1000){
    ROLLvalright=1000;
  }

  if(ROLLvalright>2000){
    ROLLvalright=2000;
  }
   
  if((ROLLcom>1480)&&(ROLLcom<1520)){
    left_prop.writeMicroseconds(pwmLeft);
    right_prop.writeMicroseconds(pwmRight);
   }
    ...
    if(ROLLcom>1520){
        left_prop.writeMicroseconds(ROLLvalleft);
        right_prop.writeMicroseconds(pwmRight);
    }
    if(ROLLcom<1480){
        left_prop.writeMicroseconds(pwmLeft);
        right_prop.writeMicroseconds(ROLLvalright);
    }
  }
